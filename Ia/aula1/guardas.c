...
int heap_isEmpty(HEAPMAX *q);  // retorna 1 ou 0
...
VERT build_vert(int id, int x1, int y1){
...
int n_guardas=0;
int s,r; //s = numero de retangulos   r = id
scanf("%d",&s);
...
for(int i=0;i<n_vert;i++){
  ...
  n_retangulos(v[i]);
  if(is_guards(v[i]==TRUE)) n_guardas++;
